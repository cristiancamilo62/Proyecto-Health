...
private static final ValidationRule<Date> instancia = new FechaNacimientoPacienteValidationRule();
	...
	private final void validarFechaActual(final Date dato) {
		if(!UtilDate.validarFechaActual(dato)) {
			throw ServiceHealthException.crear(CatalogoMensajes.obtenerContenidoMensaje(CodigoMensaje.M0000247)); 
